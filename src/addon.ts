...
// Shared TV catalog cache used by BOTH the catalog handler and the
// stream/meta handlers. Before this helper, the stream handler did a fresh
// `mergeDynamic([...staticBaseChannels])` on every play (and the meta
// handler did its own `tvChannels.find(...)` on a possibly-stale global),
// even when the catalog handler had just built the same array for free.
//
// Cache invalidates on:
//   - staticBaseChannels length change (static reload)
//   - dynamic_channels.json mtime change (writer hook)
//   - 30 min TTL (safety net)
//
// Disable entirely via NO_TV_CATALOG_CACHE=1.
const __TV_CATALOG_CACHE_TTL_MS = 30 * 60 * 1000;
function _isCatalogCacheDisabled(): boolean {
    try {
        const v = (process?.env?.NO_TV_CATALOG_CACHE ?? '0').toString().toLowerCase();
        return v === '1' || v === 'true' || v === 'yes' || v === 'on';
    } catch { return false; }
}
function getMergedTvChannelsCached(opts?: { forceRefresh?: boolean }): { channels: any[]; hit: boolean; key: string } {
    if (_isCatalogCacheDisabled() || opts?.forceRefresh) {
        try {
            loadDynamicChannels(true);
            const merged = mergeDynamic([...staticBaseChannels]);
            tvChannels = merged;
            return { channels: merged, hit: false, key: 'no-cache' };
        } catch (e) {
            console.error('❌ getMergedTvChannelsCached (no-cache) failed:', e);
            return { channels: tvChannels, hit: false, key: 'error' };
        }
    }
    ...
    if (g.__tvCatalogCache.key !== cacheKey || ageMs > __TV_CATALOG_CACHE_TTL_MS) {
        try {
            loadDynamicChannels(false);
            const merged = mergeDynamic([...staticBaseChannels]);
            g.__tvCatalogCache = { key: cacheKey, channels: merged, builtMs: Date.now() };
            tvChannels = merged;
            debugLog(`⚡ Catalog rebuild (cache miss/expired) newKey=${cacheKey} count=${merged.length}`);
            return { channels: merged, hit: false, key: cacheKey };
        } catch (e) {
            console.error('❌ getMergedTvChannelsCached (cache miss) failed:', e);
            return { channels: tvChannels, hit: false, key: cacheKey };
        }
    }
    tvChannels = g.__tvCatalogCache.channels;
    debugLog(`⚡ Catalog served from cache key=${cacheKey} count=${g.__tvCatalogCache.channels.length}`);
...
function createBuilder(initialConfig: AddonConfig = {}) {
    ...
    builder.defineCatalogHandler(async ({ type, id, extra, config: requestConfig }: { type: string; id: string; extra?: any; config?: any }) => {
        if (type === "tv") {
            ...
            // getMergedTvChannelsCached() per dettagli e invalidazione.
            const _catalog = getMergedTvChannelsCached();
            let filteredChannels = _catalog.channels;
    ...
    builder.defineMetaHandler(async ({ type, id, config: requestConfig }: { type: string; id: string; config?: any }) => {
        ...
        if (type === "tv") {
            ...
            // Usa la cache condivisa per evitare di rifare il merge dynamic
            // ad ogni meta lookup.
            const _metaCatalog = (() => {
                try { return getMergedTvChannelsCached().channels; }
                catch (e) { console.error('❌ Meta handler: getMergedTvChannelsCached failed:', e); return tvChannels; }
            })();
            const channel = _metaCatalog.find((c: any) => c.id === cleanId);
            if (channel) {
    ...
    const streamHandlerImpl = async ({
            ...
            try {
                ...
                if (type === "tv") {
                    ...
                    // Riusa la cache condivisa del catalog handler invece di
                    // fare un mergeDynamic completo su ogni stream play.
                    let _streamCatalog: any[];
                    try {
                        _streamCatalog = getMergedTvChannelsCached().channels;
                    } catch (e) {
                        console.error('❌ Stream handler: getMergedTvChannelsCached failed:', e);
                        _streamCatalog = tvChannels;
                    }
                    ...
                    const channel = _streamCatalog.find((c: any) => c.id === cleanId);
                ...
                const hasAnyProviderSetting = (() => {
                    ...
                    const providerKeys = [
                        ...
                        'guardahdEnabled', 'adnEnabled', 'guardaserieEnabled', 'vidxgoEnabled', 'guardoserieEnabled', 'guardaflixEnabled', 'cinemacityEnabled',
                ...
                if ((id.startsWith('kitsu:') || id.startsWith('mal:') || id.startsWith('tt') || id.startsWith('tmdb:')) && (trailerEnabled || animeUnityEnabled || animeSaturnEnabled || animeWorldEnabled || guardaSerieEnabled || vidxgoEnabled || guardoserieEnabled || guardaflixEnabled || guardaHdEnabled || adnEnabled || eurostreamingEnabled || toonitaliaEnabled || toonEnabled || cb01Enabled || vixsrcEnabled || cinemacityEnabled)) {
                    ...
                    if (adnEnabled && (id.startsWith('tt') || id.startsWith('tmdb:'))) {
                        scheduleProviderRun('ADN', true, async () => {
                            ...
                            return { streams: [] };
                        }, providerLabel('adn'), true, 30000);  // ADN: timeout 30s
                    }
...
// ---------------------------------------------------------------------------
// /chapta — manual captcha solver per uprot/clicka (fallback all'OCR warmup)
// ---------------------------------------------------------------------------
// Flusso:
//   GET  /chapta  -> per ognuno tra uprot/clicka: se gia' whitelistato (state
//                    file fresco) mostra spunta verde; altrimenti scarica il
//                    PNG captcha tramite il proxy slot attivo del dominio e
//                    lo mostra inline con un input numerico.
//   POST /chapta  -> riceve { uprot_sid, uprot_code, clicka_sid, clicka_code },
//                    per ciascuna coppia presente fa --submit-manual usando
//                    LO STESSO proxy slot della prepare (sticky via session).
// Errori sempre visibili nella pagina (sia prepare che submit).
{
    ...
    function _cleanupChaptaSessions(): void {
        const now = Date.now();
        for (const [sid, info] of chaptaSessions) {
            if (now - info.createdMs > CHAPTA_SESSION_TTL_MS) {
                try { if (fs.existsSync(info.filePath)) fs.unlinkSync(info.filePath); } catch { /* ignore */ }
    ...
    function _renderChapta(cards: CardData[], token: string, topMessage?: { kind: 'ok' | 'err' | 'info'; text: string }): string {
        const card = (c: CardData) => {
            ...
            const statusLine = c.whitelisted
                ? `<span style="color:#22c55e;font-weight:700">&#10003; gi&agrave; whitelistato</span>`
                  + (ageMin != null ? ` <span style="opacity:.7;font-size:.85em">(state ${ageMin}m fa)</span>` : '')
                : `<span style="color:#ef4444;font-weight:700">&times; non whitelistato</span>`;
            const proxyLine = `proxy slot attivo: <code>${_escapeHtml(c.activeSlot)}</code> &rarr; <code>${_escapeHtml(c.activeProxyHost)}</code>`;
            let body = '';
            if (c.whitelisted) {
            ...
            let submit = '';
            if (c.submitOk) {
                submit = `<p style="color:#22c55e;margin-top:.5em"><b>&#10003; submit OK</b> (guess <code>${_escapeHtml(c.submitGuess || '')}</code>, slot <code>${_escapeHtml(c.sessionProxySlot || '')}</code>) &mdash; state file scritto.</p>`;
            } else if (c.submitError) {
                submit = `<p style="color:#ef4444;margin-top:.5em"><b>&times; submit FAIL:</b> <code>${_escapeHtml(c.submitError)}</code> (slot <code>${_escapeHtml(c.sessionProxySlot || '')}</code>)</p>`;
            }
            return `
            <div style="border:1px solid #444;border-radius:10px;padding:1em;margin:1em 0;background:#181818">
        ...
        const banner = topMessage ? `
          <div style="padding:.7em 1em;border-radius:8px;margin:.5em 0;background:${
              topMessage.kind === 'ok' ? '#064e3b' : topMessage.kind === 'err' ? '#7f1d1d' : '#1e3a8a'
          };color:#fff">${_escapeHtml(topMessage.text)}</div>` : '';
    ...
    async function _buildCardForGET(
        ...
        try {
            ...
            if (!r.ok) {
                card.prepareError = r.error || 'unknown prepare error';
                return card;
            }
            ...
            try { fs.writeFileSync(filePath, JSON.stringify(r.session)); } catch (e) {
                card.prepareError = `cannot persist session: ${(e as Error).message}`;
                return card;
            }
            ...
            card.prepareError = `prepare exception: ${(e as Error).message}`;
        }
        return card;
    }
    ...
    app.post('/chapta', express.urlencoded({ extended: false }), async (req: Request, res: Response) => {
        try {
            ...
            for (const domain of ['uprot', 'clicka'] as const) {
                ...
                if (sid && !code) {
                    card.submitError = 'codice non inserito';
                    anyFail = true;
                    cards.push(card);
                    continue;
                }
                ...
                try {
                    const r = await submitManualSolve(sess.filePath, code);
                    if (r.ok) {
                        card.submitOk = true;
                        ...
                        card.submitError = r.error || 'unknown submit error';
                        card.sessionProxySlot = r.proxySlot;
                        anyFail = true;
                    }
                } catch (e) {
